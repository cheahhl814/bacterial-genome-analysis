...
| `$RUN_DIR/draft_graph.gfa` | `Flye` / `Dragonflye`            | GFA    | Assembly graph, when produced. Used by `Bandage` to verify circularity independent of the assembler's self-report. |
...
> **Preflight guard.** Before running any assembly command, verify `preflight.md` overall verdict ≥ `GO-WITH-WARNINGS` and `params.json` `$platform` is `ont` or `pacbio-hifi`. If `params.json` recommends a specific assembler (e.g. `flye` for ONT, `flye --pacbio-hifi` for HiFi, `raven` for low-RAM), use that — the preflight evidence supports the choice.
...
cp "$RUN_DIR/flye_output/assembly_graph.gfa" "$RUN_DIR/draft_graph.gfa"
```
...
cp "$RUN_DIR"/dragonflye_out/*.gfa "$RUN_DIR/draft_graph.gfa" 2>/dev/null || true
```

Autocycler's consensus step merges multiple graphs and does not emit a single representative `.gfa`; no graph check for Path A — rely on the per-assembler Flye/Raven/Miniasm graphs that fed into it if deeper inspection is needed.

#### Graph-based circularity check (Bandage)

If `$RUN_DIR/draft_graph.gfa` exists, verify circularity against the graph itself rather than trusting the assembler's self-report alone:
...
# Dead ends: 0 = fully circularized; >0 = at least one contig/plasmid is not closed.
```

Both are headless (no X server / GUI required). Skip silently if no `.gfa` was produced.

## Interpretation Guidelines

- **Circularization**: Check if the assembly reports circular chromosomes or plasmids. This is the goal for bacterial genomes. Cross-check with `Bandage info`'s dead-end count when `draft_graph.gfa` is available — the graph is ground truth, the assembler's log is a claim.
...
- [ ] The agent has verified that circularization was attempted, via `Bandage info`'s dead-end count when `draft_graph.gfa` was produced (Flye/Dragonflye paths), otherwise via the assembler's own report.
...
- `$RUN_DIR/draft_graph.gfa`, `draft_graph_info.txt`, `draft_graph.png` (Flye/Dragonflye paths only; not produced by Autocycler)
