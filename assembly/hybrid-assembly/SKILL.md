...
| `$RUN_DIR/draft_graph.gfa` | `Unicycler` / `Dragonflye` | GFA | Assembly graph, copied from tool output when produced. Used by `Bandage` to verify circularity independent of the assembler's self-report. |
...
cp "$RUN_DIR/unicycler_output/assembly.gfa" "$RUN_DIR/draft_graph.gfa"
```
*Key Flags:*
...
If `$RUN_DIR/draft_graph.gfa` exists (Unicycler/Dragonflye paths), verify circularity against the graph itself rather than trusting the assembler's self-report alone — a contig an assembler calls "circular" can still show dead ends in the graph:
...
Both are headless (no X server / GUI required). Skip silently if no `.gfa` was produced (e.g. Hybracter path).
...
- [ ] The assembly is validated for circularity via `Bandage info` on `draft_graph.gfa` (dead-end count) where a graph was produced; otherwise fall back to the assembler's own report / QUAST.
...
- `$RUN_DIR/draft_graph.gfa`, `draft_graph_info.txt`, `draft_graph.png` (Unicycler/Dragonflye paths only)
